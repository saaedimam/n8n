...
const server = new Server(
  { name: "orchestra-mcp", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
  ...
  try {
    switch (name) {
      case "scaffold":
        ...
        return {
          content: [{ type: "text", text: "ok: .orchestra ready" }]
        };
      ...
      case "plan": {
        ...
        return {
          content: [{ type: "text", text: "ok: plan.json written" }]
        };
      }
      ...
      case "split": {
        ...
        for (let i = 0; i < titles.length; i++) {
          ...
          const p = path.join(JOBS, `${branch.replace(/\//g, "-")}.json`);
          await writeJSON(p, job);
          out.push(p);
        }
        return {
          content: [{ type: "text", text: `ok: ${out.length} job(s) created\n` + out.join("\n") }]
        };
      }
      ...
      case "pr": {
        ...
        try {
          ...
          return {
            content: [{ type: "text", text: `ok: PR attempted\n${stdout || ""}` }]
          };
        } catch (e: any) {
          return {
            content: [{ type: "text", text: `err: ${e.message}` }]
          };
        }
      }
...
async function main() {
  ...
  console.error("Orchestra MCP server running on stdio");
}
